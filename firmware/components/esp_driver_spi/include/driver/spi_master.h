...
#define SPI_TRANS_DMA_BUFFER_ALIGN_MANUAL   (1<<11) ///< By default driver will automatically re-alloc a DMA buffer if needed. This disables that allocation: the caller must provide DMA-capable, address-aligned storage and, for PSRAM/cache-aligned memory, an aligned length, otherwise ESP_ERR_INVALID_ARG is returned.
 ...
 *                                 or set flag SPI_TRANS_DMA_BUFFER_ALIGN_MANUAL but a tx/rx buffer violates its DMA address/capacity or cache-alignment contract
