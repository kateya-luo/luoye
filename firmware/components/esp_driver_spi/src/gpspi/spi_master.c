...
static SPI_MASTER_ISR_ATTR esp_err_t setup_dma_priv_buffer(spi_host_t *host, uint32_t *buffer, uint32_t len, bool is_tx, uint32_t flags, uint32_t **ret_buffer)
{
    ...
    /* LUOYE_SPI_EXACT_LENGTH_DMA_BACKPORT
     *
     * Backport the release/v5.5 alignment rule to the pinned ESP-IDF 5.5.4
     * driver.  Internal ESP32-S3 DMA memory is not cache-line constrained:
     * its address must be DMA aligned, while the valid transaction length may
     * remain exact.  DMA descriptors still reserve aligned capacity and the
     * HAL still clocks trans->length/trans->rxlength bits, so this does not add
     * bytes to the SPI wire transaction.
     *
     * PSRAM and cache-aligned internal-memory targets retain the original
     * address-and-length requirement.
     */
    need_malloc |= (use_psram || host->bus_attr->cache_align_int > 1)
                       ? (((uint32_t)buffer | len) & (alignment - 1))
                       : (((uint32_t)buffer) & (alignment - 1));
    const uint32_t align_len = (len + alignment - 1) & (~(alignment - 1));
    ...
    if (need_malloc) {
        ...
        if (is_tx) {
            /* Only copy bytes owned by the caller. align_len is allocation
             * and cache-maintenance capacity, not readable source length. */
    ...
    *ret_buffer = buffer;
    if (use_psram || (host->bus_attr->cache_align_int > 1)) {
        esp_err_t ret = esp_cache_msync((void *)buffer, align_len, is_tx ? (ESP_CACHE_MSYNC_FLAG_DIR_C2M | ESP_CACHE_MSYNC_FLAG_UNALIGNED) : ESP_CACHE_MSYNC_FLAG_DIR_M2C);
