...
/* Luoye uses the SDSPI host, whose transaction layer owns a permanent
 * internal DMA buffer and copies non-DMA/PSRAM payloads into it one sector at
 * a time. The generic sdmmc wrapper must therefore trust an aligned SPI-host
 * buffer instead of allocating a second temporary 512-byte DMA bounce buffer
 * at runtime. Native SDMMC hosts retain the upstream IDF 5.5.4 rules. */
#define LUOYE_SDSPI_HOST_OWNS_DMA_STAGING 1
static bool sdmmc_buffer_can_transfer_directly(sdmmc_card_t* card,
        const void* buffer, size_t size)
{
    if (!card->host.check_buffer_alignment(card->host.slot, buffer, size)) {
        return false;
    }
    if (host_is_spi(card)) {
        return true;
    ...
    return !esp_ptr_external_ram(buffer);
#else
    return true;
#endif
}
        ...
        size_t start_block, size_t block_count)
{
    ...
    bool can_transfer_directly = sdmmc_buffer_can_transfer_directly(
            card, src, block_size * block_count);
        ...
        size_t start_block, size_t block_count)
{
    ...
    size_t block_size = card->csd.sector_size;
    bool can_transfer_directly = sdmmc_buffer_can_transfer_directly(
            card, dst, block_size * block_count);
