...
esp_err_t sdspi_host_do_transaction(int slot, sdmmc_command_t *cmdinfo)
{
    ...
    /* LUOYE_SDSPI_NO_TRANSIENT_DMA_BOUNCE
     *
     * The uploader intentionally runs on a PSRAM stack. A merely
     * WORD_ALIGNED local command therefore still lives in PSRAM and makes
     * spi_master allocate private internal TX and RX buffers for every SD
     * command. Keep one internal DMA-capable command object instead. s_lock
     * serializes every public SDSPI transaction before this object is used. */
    static DMA_ATTR sdspi_hw_cmd_t hw_cmd;
