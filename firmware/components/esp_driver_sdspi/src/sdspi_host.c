...
/* The DMA descriptor may reserve up to 516 bytes, but the SDSPI transaction
 * must keep the exact upstream wire length.  In particular, the final block
 * is 512 data bytes plus two CRC bytes (514), because some cards are confused
 * by additional clocks.  The project-local esp_driver_spi backport permits an
 * aligned internal DMA buffer to carry that exact, non-word-sized length
 * without allocating a temporary bounce buffer. */
#define SDSPI_DMA_ALIGNMENT     4
...
typedef struct {
    ...
    /// Permanent internal DMA buffer used for all SDSPI payload and small
    /// command-support transactions. Allocated before the card is mounted.
} slot_info_t;
...
static esp_err_t get_block_buf(slot_info_t *slot, uint8_t **out_buf)
{
    if (slot->block_buf == NULL) {
        slot->block_buf = heap_caps_aligned_alloc(SDSPI_DMA_ALIGNMENT,
                                                  SDSPI_BLOCK_BUF_SIZE,
                                                  MALLOC_CAP_INTERNAL |
                                                  MALLOC_CAP_DMA);
        if (slot->block_buf == NULL) {
        ...
        ESP_LOGI(TAG,
                 "LY|SDSPI_DMA|mode=aligned_static_exact_wire bytes=%u alignment=%u address=%p internal=%d dma=%d",
                 (unsigned)SDSPI_BLOCK_BUF_SIZE,
                 (unsigned)SDSPI_DMA_ALIGNMENT, slot->block_buf,
                 esp_ptr_in_dram(slot->block_buf),
                 esp_ptr_dma_capable(slot->block_buf));
    }
    assert(((uintptr_t)slot->block_buf & (SDSPI_DMA_ALIGNMENT - 1)) == 0);
...
{
    uint8_t *data = NULL;
    if (get_block_buf(slot, &data) != ESP_OK) {
        return;
    }
    data[0] = SDSPI_MOSI_IDLE_VAL;
    spi_transaction_t t = {
...
static void go_idle_clockout(slot_info_t *slot)
{
    ...
    memset(data, SDSPI_MOSI_IDLE_VAL, 10);
    spi_transaction_t t = {
        .flags = SPI_TRANS_DMA_BUFFER_ALIGN_MANUAL,
        .length = 10 * 8,
        .tx_buffer = data,
...
esp_err_t sdspi_host_init_device(const sdspi_device_config_t* slot_config, sdspi_dev_handle_t* out_handle)
{
    ...
    *slot = (slot_info_t) {
    ...
    /* Reserve the only SDSPI block bounce buffer before mounting the card.
     * Upload-time reads must never depend on a transient internal allocation. */
    uint8_t *reserved_block_buf = NULL;
    ret = get_block_buf(slot, &reserved_block_buf);
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "LY|SDSPI_DMA|event=reserve_failed bytes=%u",
                 (unsigned)SDSPI_BLOCK_BUF_SIZE);
        goto cleanup;
    }
    assert(reserved_block_buf == slot->block_buf);
...
static esp_err_t poll_busy(slot_info_t *slot, int timeout_ms, bool polling)
{
    ...
    uint8_t *data = NULL;
    esp_err_t ret = get_block_buf(slot, &data);
    if (ret != ESP_OK) {
        return ret;
    }
    spi_transaction_t t = {
...
static esp_err_t poll_data_token(slot_info_t *slot, uint8_t *extra_ptr, size_t *extra_size, int timeout_ms)
{
    ...
    int64_t t_end = esp_timer_get_time() + timeout_ms * 1000;
    do {
        memset(data, SDSPI_MOSI_IDLE_VAL, 8);
        ret = spi_device_polling_transmit(slot->spi_handle, &t);
        if (ret != ESP_OK) {
            return ret;
        }
        ...
        for (size_t byte_idx = 0; byte_idx < 8; byte_idx++) {
            uint8_t rd_data = data[byte_idx];
            if (rd_data == TOKEN_BLOCK_START) {
                found = true;
                memcpy(extra_ptr, data + byte_idx + 1, 8 - byte_idx - 1);
                *extra_size = 8 - byte_idx - 1;
                break;
            }
                                           ...
                                           uint8_t *data, uint32_t rx_length, bool need_stop_command)
{
    ...
    while (rx_length > 0) {
        ...
        if (extra_data_size > expected_data_size) {
            ESP_LOGE(TAG,
                     "invalid read look-ahead: extra=%u expected=%u",
                     (unsigned)extra_data_size,
                     (unsigned)expected_data_size);
            return ESP_ERR_INVALID_SIZE;
        }

        /* Preserve any payload byte captured after the previous block's
         * look-ahead token before get_block_buf returns that same buffer and
         * we fill it with MOSI idle bytes. Copying this later corrupts the
         * first byte of the next block and deterministically breaks CRC. */
        if (extra_data_size) {
        ...
        // Receive the exact upstream wire length. The permanent buffer has
        // padded DMA capacity, but extra clocks here would move CRC bytes
        // across multi-block boundaries on affected cards.
        const size_t receive_extra_bytes = (rx_length > SDSPI_MAX_DATA_LEN) ? 4 : 2;
        const size_t receive_bytes = will_receive + receive_extra_bytes;
        assert(receive_bytes <= SDSPI_BLOCK_BUF_SIZE);
        memset(rx_data, 0xff, SDSPI_BLOCK_BUF_SIZE);
    ...
    if (need_stop_command) {
        ...
        // card to process it
        /* sdspi_host_do_transaction() holds the global SDSPI lock here, so a
         * single internal stop-command object is safe and cannot live on the
         * uploader's PSRAM stack. */
        static DMA_ATTR sdspi_hw_cmd_t stop_cmd;
        assert(esp_ptr_in_dram(&stop_cmd));
        assert(esp_ptr_dma_capable(&stop_cmd));
                                            ...
                                            const uint8_t *data, uint32_t tx_length, bool multi_block, bool stop_trans)
{
    ...
    while (tx_length > 0) {
        ...
        ret = get_block_buf(slot, &dma_buffer);
        if (ret != ESP_OK) {
            return ret;
        }
        // Write block start token
        dma_buffer[0] = start_token;
        ...
        if (!esp_ptr_dma_capable(tx_data) ||
                (((uintptr_t)tx_data & (SDSPI_DMA_ALIGNMENT - 1)) != 0)) {
            // If the pointer can't be used with DMA, copy data into a new buffer
            memcpy(dma_buffer, tx_data, will_send);
            tx_data = dma_buffer;
        }
        ...
        memset(dma_buffer, SDSPI_MOSI_IDLE_VAL, size_crc_response);
        memcpy(dma_buffer, &crc, sizeof(crc));
    ...
    if (stop_trans) {
        ...
        dma_buffer[0] = TOKEN_BLOCK_STOP_WRITE_MULTI;
        dma_buffer[1] = SDSPI_MOSI_IDLE_VAL;
